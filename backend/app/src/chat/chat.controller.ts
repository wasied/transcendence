...
*/
	...
	async setPunishment(
		...
		const admin_id = await this.chatService.findChatroomUserId(body.chatroom_id, request.user.id);
		const target_id = await this.chatService.findChatroomUserId(body.chatroom_id, body.target_id);
