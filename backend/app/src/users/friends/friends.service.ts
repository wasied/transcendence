...
export class FriendsService {
	...
	async findUserFriends(user_id: number): Promise<User[]> {
		const result = await dbClient.query(
			`SELECT *	FROM users
						WHERE id IN (
	...
	async create(user_uid1: number, user_uid2: number): Promise<void> {
		const result = await dbClient.query(
	...
	async delete(user_id: number, friendship_id: number): Promise<void> {
		const result = await dbClient.query(
			`IF EXISTS (
				SELECT *	FROM friends
							WHERE id = $1
